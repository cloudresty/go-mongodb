...
if !health.IsHealthy {
    log.Printf("MongoDB unhealthy: %s", health.Error)
}

// Connection status
if client.IsConnected() {
    log.Println("MongoDB connection is active")
}
```
...
err := client.WithTransaction(ctx, func(ctx context.Context) error {
    ...
    _, err := collection.UpdateOne(ctx,
        map[string]any{"account_id": "account_1"},
    ...
    // Credit account
    _, err = collection.UpdateOne(ctx,
        map[string]any{"account_id": "account_2"},
        map[string]any{"$inc": map[string]any{"balance": 100}},
    )
    return err
})
...
config := &mongodb.Config{
    // Connection timeouts
    ConnectTimeout:      30 * time.Second,  // Initial connection
    ServerSelectTimeout: 10 * time.Second,  // Server discovery
    SocketTimeout:       60 * time.Second,  // Operation timeout

    // Pool timeouts
    MaxIdleTime:         5 * time.Minute,   // Connection idle timeout
    MaxConnIdleTime:     10 * time.Minute,  // Maximum connection age
}
```
...
clientB, _ := mongodb.NewClientWithPrefix("SERVICE_B_")
...
config := &mongodb.Config{
    ...
    MaxIdleTime:     5 * time.Minute,  // Idle connection timeout
    MaxConnIdleTime: 30 * time.Minute, // Maximum connection age
}
```
...
config := &mongodb.Config{
    ...
    CompressionAlgorithm: "zstd", // snappy, zlib, zstd
}
...
config := &mongodb.Config{
    ...
    WriteConcern:   "majority",         // majority, 1, 2, 3, etc.
    ReadConcern:    "local",           // local, available, majority, linearizable
}
```
