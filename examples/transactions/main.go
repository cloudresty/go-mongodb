...
func main() {
	...
	_, _ = orders.DeleteMany(ctx, bson.M{})
	_, _ = inventory.DeleteMany(ctx, bson.M{})
	...
	result, err := session.WithTransaction(ctx, func(sessCtx context.Context) (any, error) {
		...
		update := bson.M{"$inc": bson.M{"quantity": -order.Quantity}}
		...
		var inv Inventory
		err = inventory.FindOne(sessCtx, filter).Decode(&inv)
		if err != nil {
	...
	err = inventory.FindOne(ctx, bson.M{"product": "laptop"}).Decode(&finalInventory)
	if err != nil {
	...
	orderCount, err = orders.CountDocuments(ctx, bson.M{})
	if err != nil {
